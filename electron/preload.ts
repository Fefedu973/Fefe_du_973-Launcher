...
contextBridge.exposeInMainWorld('api', {
  auth: {
    loginMicrosoft: (): Promise<IAuthResponse> => ipcRenderer.invoke('auth:login_microsoft'),
  ...
  game: {
    ...
    patchDebug: (callback: (value: PatcherEvents['patch_debug'][0]) => void) =>
      ipcRenderer.on('game:patch_debug', (_event, value) => callback(value)),
    launchError: (callback: (message: string) => void) => ipcRenderer.on('game:launch_error', (_event, message) => callback(message))
  },
  skin: {
  ...
  bootstraps: {
    ...
    error: (callback: (value: BootstrapEvents['bootstrap_error'][0]) => void) =>
