...
import { ADMINTOOL_URL, GAME_ROOT, MINECRAFT_SERVER } from '../const'
...
export function registerLauncherHandlers(mainWindow: BrowserWindow) {
  ipcMain.handle('game:launch', async (_event, payload: { account: Account; settings: IGameSettings; profileSlug: string }) => {
    if (launchInProgress) {
      return { success: false, error: 'Un lancement est déjà en cours.' }
    }

    const { account, settings, profileSlug } = payload
    if (!profileSlug) {
      return { success: false, error: 'Aucun profil de jeu n’est sélectionné.' }
    }
    ...
    const launcher = new Launcher({
      ...
      storage: 'isolated',
      minecraft: {
        args: ['--server', MINECRAFT_SERVER.host, '--port', MINECRAFT_SERVER.port.toString()]
      },
      cleaning: {
        enabled: true
      },
    ...
    const reportLaunchError = (err: unknown) => {
      launchInProgress = false
      logger.error('Launcher error:', err)
      const message = err instanceof Error ? err.message : 'Erreur inconnue du launcher'
      mainWindow.webContents.send('game:launch_error', message)
    }

    void launcher.launch().catch(reportLaunchError)
    return { success: true }
  })
}
